    ...
    <main className="flex min-h-screen flex-col items-center justify-center bg-surface px-spacing-xl">
      <div className="animate-fade-up w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="mt-spacing-3xl flex flex-col gap-spacing-md">
          ...
          <button
            type="submit"
            className="rounded-radius-md bg-primary-500 px-spacing-xl py-spacing-md text-[var(--font-size-base)] font-semibold text-white transition-all duration-[var(--duration-normal)] ease-[var(--ease-out)] hover:bg-primary-600 active:scale-[0.98]"
          >
            Send magic link
          </button>
          {error && (
            <p className="text-center text-[var(--font-size-sm)] text-danger-500">{error}</p>
          )}
        </form>
      </div>
    </main>
