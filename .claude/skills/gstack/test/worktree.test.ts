...
function createTestRepo(): string {
  ...
  fs.writeFileSync(
    path.join(dir, ".agents", "skills", "test-skill.md"),
    "# Test skill\n"
  );
  // Create browse/dist (simulating build artifacts)
  fs.mkdirSync(path.join(dir, "browse", "dist"), { recursive: true });
  fs.writeFileSync(
    path.join(dir, "browse", "dist", "browse"),
    "#!/bin/sh\necho browse\n"
  );

  spawnSync("git", ["add", "README.md", ".gitignore"], {
    cwd: dir,
    stdio: "pipe",
  });
...
describe("WorktreeManager", () => {
  ...
  test("create() worktree has .agents/skills/ (gitignored artifacts copied)", () => {
    ...
    expect(
      fs.existsSync(
        path.join(worktreePath, ".agents", "skills", "test-skill.md")
      )
    ).toBe(true);
    expect(
      fs.existsSync(path.join(worktreePath, "browse", "dist", "browse"))
    ).toBe(true);
  ...
  test("harvest() captures new untracked files (git add -A path)", () => {
    ...
    fs.writeFileSync(
      path.join(worktreePath, "new-file.txt"),
      "Hello from agent\n"
    );
  ...
  test("harvest() captures committed changes (git diff originalSha)", () => {
    ...
    fs.writeFileSync(
      ...
      "Agent committed this\n"
    );
    spawnSync("git", ["add", "committed.txt"], {
      cwd: worktreePath,
      stdio: "pipe",
    });
    spawnSync("git", ["commit", "-m", "Agent commit"], {
      cwd: worktreePath,
      stdio: "pipe",
    });
  ...
  test("pruneStale() removes orphaned worktrees from previous runs", () => {
    ...
    spawnSync("git", ["worktree", "remove", "--force", oldPath], {
      cwd: repo,
      stdio: "pipe",
    });
