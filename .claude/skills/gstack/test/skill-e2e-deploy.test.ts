...
describeIfSelected(
  "Land-and-Deploy skill E2E",
  ["land-and-deploy-workflow"],
  () => {
    let landDir: string;

    beforeAll(() => {
      landDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "skill-e2e-land-deploy-")
      );
      ...
      copyDirSync(
        path.join(ROOT, "land-and-deploy"),
        path.join(landDir, "land-and-deploy")
      );
    });
...
Do NOT use AskUserQuestion. Do NOT run gh or fly commands.`,
        ...
        if (fs.existsSync(claudeMd)) {
          ...
          const hasFly =
            content.toLowerCase().includes("fly") ||
            content.toLowerCase().includes("test-app");
...
Just create the directory structure and report files showing the correct schema.`,
      ...
      expect(
        fs.existsSync(path.join(canaryDir, ".gstack", "canary-reports"))
      ).toBe(true);
...
Just create the files showing the correct schema and report format.`,
      ...
      expect(
        fs.existsSync(path.join(benchDir, ".gstack", "benchmark-reports"))
      ).toBe(true);
...
describeIfSelected("Setup-Deploy skill E2E", ["setup-deploy-workflow"], () => {
  ...
  beforeAll(() => {
    setupDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "skill-e2e-setup-deploy-")
    );
    const run = (cmd: string, args: string[]) =>
    ...
    copyDirSync(
      path.join(ROOT, "setup-deploy"),
      path.join(setupDir, "setup-deploy")
    );
  });
