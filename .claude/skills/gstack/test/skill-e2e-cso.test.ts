...
- Save the report to .gstack/security-reports/.`,
    ...
    expect(
      output.includes(".env") &&
        (output.includes("tracked") || output.includes("gitignore"))
    ).toBe(true);

    // Should produce a findings table
    expect(
      output.includes("security findings") ||
        output.includes("SECURITY FINDINGS")
    ).toBe(true);
...
- The webhook.ts file was added on this branch — it should be analyzed.`,
    ...
    expect(
      output.includes("webhook") &&
        (output.includes("signature") || output.includes("verify"))
    ).toBe(true);
...
describeIfSelected("CSO v2 — infra scope", ["cso-infra-scope"], () => {
  ...
  beforeAll(() => {
    csoInfraDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "skill-e2e-cso-infra-")
    );
    ...
    fs.mkdirSync(path.join(csoInfraDir, ".github", "workflows"), {
      recursive: true,
    });
    fs.writeFileSync(
