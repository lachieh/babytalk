...
const TEST_KEY = crypto.pbkdf2Sync(
  TEST_PASSWORD,
  "saltysalt",
  1003,
  16,
...
const LINUX_V10_KEY = crypto.pbkdf2Sync(
  LINUX_V10_PASSWORD,
  "saltysalt",
  1,
  16,
...
const LINUX_V11_KEY = crypto.pbkdf2Sync(
  LINUX_V11_PASSWORD,
  "saltysalt",
  1,
  16,
...
function createMacFixtureDb() {
  ...
  insert.run(
    ...
    "sess",
    "",
    encryptCookieValue("session-value"),
    "/",
    0,
    1,
    1,
...
function createLinuxFixtureDb() {
  ...
  insert.run(
    ...
    "sid",
    "",
    encryptCookieValue("linux-v10-value", {
      key: LINUX_V10_KEY,
      prefix: "v10",
    }),
    "/",
  ...
  insert.run(
    ...
    "auth",
    "",
    encryptCookieValue("linux-v11-value", {
      key: LINUX_V11_KEY,
      prefix: "v11",
    }),
    "/",
    ...
    if (Array.isArray(cmd) && cmd[0] === "secret-tool" && cmd[1] === "lookup") {
      return {
        ...
        stdout: new ReadableStream({
          start(controller) {
            controller.enqueue(
              new TextEncoder().encode(LINUX_V11_PASSWORD + "\n")
            );
            controller.close();
          },
        }),
...
async function withInstalledProfile<T>(
  ...
  const backupPath = path.join(
    profileDir,
    `Cookies.backup-${crypto.randomUUID()}`
  );
...
describe("Cookie Import Browser", () => {
  ...
  describe("Fixture DB Structure", () => {
    test("fixture DB has correct domain counts", () => {
      ...
      const counts = Object.fromEntries(
        rows.map((r: any) => [r.host_key, r.count])
      );
  ...
  describe("sameSite Mapping", () => {
    test("maps sameSite values correctly", () => {
      ...
      // samesite=0 → None
      const none = db
        .query(`SELECT samesite FROM cookies WHERE name = 'user_token'`)
        .get() as any;
      expect(none.samesite).toBe(0);

      // samesite=1 → Lax
      const lax = db
        .query(`SELECT samesite FROM cookies WHERE name = 'session_id'`)
        .get() as any;
      expect(lax.samesite).toBe(1);

      // samesite=2 → Strict
      const strict = db
        .query(`SELECT samesite FROM cookies WHERE name = 'theme'`)
        .get() as any;
  ...
  describe("Profile Validation", () => {
    test("rejects path traversal in profile names", () => {
      ...
      expect(() => listDomains("chrome", "Default/../../etc")).toThrow(
        /Invalid profile/
      );
    });
