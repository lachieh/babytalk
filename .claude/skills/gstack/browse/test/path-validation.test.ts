...
describe("validateOutputPath", () => {
  ...
  it("allows paths within cwd", () => {
    expect(() =>
      validateOutputPath(`${process.cwd()}/output.png`)
    ).not.toThrow();
  });

  it("blocks paths outside safe directories", () => {
    expect(() => validateOutputPath("/etc/cron.d/backdoor.png")).toThrow(
      /Path must be within/
    );
  });

  it("blocks /tmpevil prefix collision", () => {
    expect(() => validateOutputPath("/tmpevil/file.png")).toThrow(
      /Path must be within/
    );
  });

  it("blocks home directory paths", () => {
    expect(() => validateOutputPath("/Users/someone/file.png")).toThrow(
      /Path must be within/
    );
  });

  it("blocks path traversal via ..", () => {
    expect(() => validateOutputPath("/tmp/../etc/passwd")).toThrow(
      /Path must be within/
    );
  });
});
...
describe("validateReadPath", () => {
  ...
  it("blocks absolute paths outside safe directories", () => {
    expect(() => validateReadPath("/etc/passwd")).toThrow(
      /Absolute path must be within/
    );
  });

  it("blocks /tmpevil prefix collision", () => {
    expect(() => validateReadPath("/tmpevil/file.js")).toThrow(
      /Absolute path must be within/
    );
  });
  ...
  it("blocks nested path traversal", () => {
    expect(() => validateReadPath("src/../../etc/passwd")).toThrow(
      /Path traversal/
    );
  });
});
