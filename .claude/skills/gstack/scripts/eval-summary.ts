...
for (const file of files) {
  try {
    results.push(
      JSON.parse(fs.readFileSync(path.join(EVAL_DIR, file), "utf-8"))
    );
  } catch {
    continue;
  }
...
const avgE2ECost =
  e2eRuns.length > 0
    ? e2eRuns.reduce((s, r) => s + r.total_cost_usd, 0) / e2eRuns.length
    : 0;
const avgJudgeCost =
  judgeRuns.length > 0
    ? judgeRuns.reduce((s, r) => s + r.total_cost_usd, 0) / judgeRuns.length
    : 0;

// Duration + turns from E2E runs
const avgE2EDuration =
  e2eRuns.length > 0
    ? e2eRuns.reduce((s, r) => s + (r.total_duration_ms || 0), 0) /
      e2eRuns.length
    : 0;
...
const testEfficiency = new Map<
  string,
  { turns: number[]; durations: number[]; costs: number[] }
>();
for (const r of e2eRuns) {
...
const branchStats = new Map<
  string,
  { runs: number; avgDetection: number; detections: number[] }
>();
for (const r of e2eRuns) {
...
if (testEfficiency.size > 0 && e2eRuns.length >= 2) {
  ...
  for (const [name, stats] of sorted) {
    const avgT = Math.round(
      stats.turns.reduce((a, b) => a + b, 0) / stats.turns.length
    );
    ...
    const label =
      name.length > 30 ? name.slice(0, 27) + "..." : name.padEnd(30);
    console.log(
      `    ${label}  $${avgC}  ${avgT}t  ${avgD}s  (${stats.turns.length} runs)`
    );
  }
  console.log("─".repeat(70));
}
...
if (branchStats.size > 0) {
  ...
  const sorted = [...branchStats.entries()].sort(
    (a, b) => b[1].avgDetection - a[1].avgDetection
  );
  for (const [branch, stats] of sorted) {
    const det =
      stats.detections.length > 0
        ? ` avg det: ${stats.avgDetection.toFixed(1)}`
        : "";
